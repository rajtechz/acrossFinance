...
import PaymentHistoryHook from "./PaymentHistoryHook";
const PartialHistory = () => {
  ...
  const columns = [
    ...
    { name: "Schedule ID", selector: (row) => row.srnNo, width: "120px" },
    { name: "Batch No", selector: (row) => row.name },
    ...
    { name: "Case Count", selector: (row) => row.batchID, width: "150px" },
    { name: "Invoice No", selector: (row) => row.name },

    { name: "Invoice Date", selector: (row) => row.batchDate, width: "150px" },
    {
      name: "Invoice Amount",
...

    {
      name: "Invoice Status",
      cell: (row, index) => (
        <Chip label="Approved" color="error" style={{ borderRadius: "20px" }} />
      ),
      width: "180px",
    },
    {
